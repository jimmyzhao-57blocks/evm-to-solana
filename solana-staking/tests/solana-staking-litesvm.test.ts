...
describe("solana-staking with LiteSVM (Time Simulation)", () => {
  ...
  describe("Claim Rewards with Time Manipulation", () => {
    it("should calculate rewards correctly after time advancement", async () => {
      ...
      const currentTS = currentTimestamp();
      const fiveDaysLater = currentTS + 5 * SECONDS_IN_A_DAY;
      setNextBlockTimestamp(fiveDaysLater);
      console.log(
        `Time advanced from ${currentTS} to ${fiveDaysLater} (5 days)`
      );
      ...
      // staked: 100 tokens, rate: 5% (500 basis points), days: 5
      // rewards = (100 * 500 * 5) / 10000 = 25 tokens
    ...
    it("should not generate rewards for partial days", async () => {
      // Advance time by 12 hours (0.5 days)
      const currentTS = currentTimestamp();
      const halfDayLater = currentTS + 12 * 3600; // 12 hours
      setNextBlockTimestamp(halfDayLater);

      // Get initial balance
      const beforeAccount = getAccount(provider, userRewardToken);
      const beforeBalance = Number(beforeAccount.amount);
      ...
      // Check rewards
      const afterAccount = getAccount(provider, userRewardToken);
      const rewardsReceived = Number(afterAccount.amount) - beforeBalance;

      // Since the contract calculates days as (duration / 86400), 12 hours = 0 days
      expect(rewardsReceived).to.equal(0);
    });

    it("should handle multiple claims over different time periods", async () => {
      // TODO: Test multiple time advances and claims
    });
  });
